...
function [matrixProfile, profileIndex, motifIdxs, discordIdx, mainWindow] = ...
    interactiveMatrixProfileVer2_sublenNorm(data, subLen, proOffset)
...
mainWindow.fig = figure('name', titleTxt, ...
    'visible', 'off', 'toolbar', 'none', 'ResizeFcn', @mainResize,'outerposition',[0 0 1440,1000]); %Ryan change
...
% proOffset = 0;%int32(subLen/2); %Ryan add
...
% matrixProfile = zeros(proLen, 1); %for sum
matrixProfile = inf(proLen, 1); %for min
...
firstUpdate = true;
timer = tic();
iterationNormalizer = 1/length(idxOrder);
for i = 1:length(idxOrder)
    ...
    elseif anytimeMode == 2
        ...
        distProfile = abs(distProfile);
%         distProfile = distProfile/subLen;
%         distProfile = sqrt(distProfile);
        distProfile = distProfile./(subLen*log(subLen)*0.76);
% %         distProfile = distProfile./0.76;
        
        pos1 = idx:proLen;
        pos2 = 1:proLen - idx + 1;
        
%         matrixProfile(pos1) = matrixProfile(pos1) + distProfile.*iterationNormalizer;
%         matrixProfile(pos2) = matrixProfile(pos2) + distProfile.*iterationNormalizer;
    ...
    prefilePlot = plot(1+proOffset:proLen+proOffset, matrixProfile, ...
        'b', 'parent', mainWindow.profileAx); %Ryan add proOffset
