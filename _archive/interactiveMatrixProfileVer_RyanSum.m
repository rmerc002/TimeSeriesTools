...
function [matrixProfile, profileIndex, motifIdxs, discordIdx, mainWindow] = ...
    interactiveMatrixProfileVer_RyanSum(data, subLen, proOffset)
...
matrixProfile = zeros(proLen, 1); %ryan
...
for i = 1:length(idxOrder)
    ...
    if anytimeMode == 1
        ...
        distProfile = 1./distProfile; %Ryan
        distProfile = distProfile.*normalizer;
        
        distProfile(isSkip) = 0;%inf; %Ryan
        excZoneStart = max(1, idx - excZoneLen);
        excZoneEnd = min(proLen, idx + excZoneLen);
        distProfile(excZoneStart:excZoneEnd) = 0;%inf;%Ryan
        
        matrixProfile = matrixProfile + distProfile;
%         fprintf("here %d\n",i);
        updatePos = distProfile < matrixProfile;
        profileIndex(updatePos) = idx;
%         matrixProfile(updatePos) = distProfile(updatePos);
        [~, profileIndex(idx)] = max(matrixProfile); %Ryan
    elseif anytimeMode == 2
