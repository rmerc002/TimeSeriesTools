function  [fullMP, dataNoise] = fullMP_addNoise(dataOrig)
...
dataNoise(~indices) = rand(1,sum(~indices))*dataRange - dataRange/2;


range = [];
startLen = 10;
index = startLen;
while index < ceil(length(dataOrig)/7)
    range = [range,index];
    index = index + max(1,ceil(sqrt(index)));
end
endLen = range(end);
fullMP = zeros(endLen,length(dataNoise));
for subLen = range
