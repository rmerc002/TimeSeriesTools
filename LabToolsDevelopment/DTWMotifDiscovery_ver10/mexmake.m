...
%filelist = './selfjoin.c ./allocator.c ./cov.c';
...
if strcmp(cc.Manufacturer, 'GNU') || strcmp(cc.Manufacturer, 'Apple')  
    ...
    flags = 'COPTIMFLAGS=-O3 -march=native -funroll-loops -fwrapv -ffp-contract=fast';
...
elseif strcmp(cc.Manufacturer, 'Microsoft')
    ...
    % If you're using Windows with MSVC and these settingsc are problematic,
    % you're probably using an old cpu model or an older OS. 
    % remove the /Arch:AVX2 and it should work
    flags = 'COPTIMFLAGS=/O2 /arch:AVX2 /fp:fast /GL';
else
