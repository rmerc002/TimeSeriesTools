function  [magicMP, profileIndices,scaleSeries, subLenSeries] = magicMatrixProfileAB_Scale(dataA,dataB, subLen, startScale, endScale, numScales)
if size(dataA,1) > 1
    dataA = dataA';
end
...
subLenSeries = subLen*ones(size(scaleSeries));
...
%     [matrixProfile, profileIndex] = prescrimp(dataA',dataA', subLen, 1,ceil(length(dataA)/10));
    ...
    [matrixProfile,profileIndex] = mpx_AB(dataA',dataBScaled',subLen);
