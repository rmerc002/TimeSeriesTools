function [mp_a, mp_b, mpi_a, mpi_b] = mpx_ABBA_frequencyV001(a, b, w, numThresholds)
...
mp_a = repmat(0, numThresholds,length(a) - w + 1);
mp_b = repmat(0, numThresholds,length(b) - w + 1);
mp_a(:,~isvalid_a) = NaN;
mp_b(:,~isvalid_b) = NaN;
mpi_a = NaN(length(a) - w + 1, 1);
mpi_b = NaN(length(b) - w + 1, 1);

[mp_a, mp_b, mpi_a, mpi_b] = run_mpx(a, b, mu_a, mu_b, invn_a, invn_b, df, dg, dx, dy, mp_a, mp_b, mpi_a, mpi_b, w, numThresholds);
[mp_b, mp_a, mpi_b, mpi_a] = run_mpx(b, a, mu_b, mu_a, invn_b, invn_a, dx, dy, df, dg, mp_b, mp_a, mpi_b, mpi_a, w, numThresholds);

for ii = 1:numThresholds
    mp_a(ii,:) = mp_a(ii,:)./length(mp_a(ii,:));
    mp_b(ii,:) = mp_b(ii,:)./length(mp_b(ii,:));
end
...
if first_a ~= 1
    mp_a = [NaN(numThresholds, first_a - 1), mp_a];
    mpi_a = [NaN(numThresholds, first_a - 1), mpi_a];
    % offset indices
    mpi_b = mpi_b + (first_a - 1);
end

if first_b ~= 1
    mp_b = [NaN(numThresholds, first_b - 1), mp_b];
    mpi_b = [NaN(numThresholds, first_b - 1), mpi_b];
    % offset indices
    mpi_a = mpi_a + (first_b - 1);
end

if last_a ~= subcount_a
    mp_a = [mp_a, NaN(numThresholds, subcount_a - length(mp_a))]; 
    mpi_a = [mpi_a, NaN(numThresholds, subcount_a - length(mp_a))];
end

if last_b ~= subcount_b
    mp_b = [mp_b, NaN(numThresholds, subcount_b - length(mp_b))]; 
    mpi_b = [mpi_b, NaN(numThresholds, subcount_b - length(mp_b))];
end
...
function [mpa, mpb, mpia, mpib] = run_mpx(a, b, mua, mub, invna, invnb, df, dg, dx, dy, mpa, mpb, mpia, mpib, w, numThresholds)
amx = length(a) - w + 1;
bmx = length(b) - w + 1;
...
for ia = 1 : amx
    ...
    for ib = 1 : mx
        ...
        thresholdIndices = c_cmp > thresholdVals;
        mpa(thresholdIndices, ib + ia - 1) = mpa(thresholdIndices, ib + ia - 1) + 1;
        mpb(thresholdIndices, ib) = mpb(thresholdIndices, ib) + 1;
