function [TDProfile] = mpx_v3(timeseries, minlag, subseqlen, plot_output)
...
if subcount < minlag
    ...
    TDProfile = inf(subcount, numBins);
...
TDProfile = repmat(0, subcount, numBins);
TDProfile(~isvalidwindow,:) = NaN;
matrixProfileIdx = NaN(subcount, 1);

weights = linspace(2,0,subcount)';
...
%         end
        ...
        elementTD = abs(row-col);
        score = max(0, corr_);

        binIndex = ceil(elementTD * numBins/subcount);

        
        TDProfile(row,binIndex) = TDProfile(row,binIndex) + score;%/weights(elementTD);
%         TDProfile(col,binIndex) = TDProfile(col,binIndex) + score/weights(elementTD);

        
    end
end

TDProfile = TDProfile ./ subcount;
% TDProfile = TDProfile ./max(TDProfile(:,1:end-7),[],'all');
% TDProfile(:,end-7:end) = [];


% TDProfile = TDProfile.*weights;


% Max caps anything that rounds inappropriately. This can hide potential
% issues at times, but it's fairly rare.
% TDProfile = max(0, (1 - TDProfile), 'includenan');
