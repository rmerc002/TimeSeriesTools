...
%%
function [similarityMatrix, paa_factor, max_length] = SPLAT(timeSeriesA, subseqLen, timeSeriesB, plotting, multiresolution, calibration,n_max)
...
if ~multiresolution
    max_length = inf;
elseif ~exist('calibration','var') || (~(calibration) && ~exist('n_max','var'))
    max_length = 10000;
elseif ~(calibration)
    max_length = n_max;
else
...
histminimum = min(similarityMatrix,[],'all');
for rr = 1:size(similarityMatrix,1)
    ...
    similarityMatrix(startIndex:endIndex,rr) = histminimum;
end
...
if plotting
    ...
    portionA = length(timeSeriesA)/length(timeSeriesB);
    if portionA > 1
        widthA = 0.6;
        heigthB = 0.7 / portionA;
    elseif portionA < 1
        widthA = 0.6 * portionA;
        heigthB = 0.7;
    else
    ...
    pos2 = [0.2 0.8 - heigthB widthA heigthB];
    ...
    pos3 = [0.14 0.8 - heigthB 0.05 heigthB];
