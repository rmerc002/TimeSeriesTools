...
plato = ContrastProfile(pos, neg, subLength, false);


%%% Remove the training data in order to test
testIndices = true(length(T),1);
testIndices(startIndexPos:endIndexPos) = false;
testIndices(startIndexNeg:endIndexNeg) = false;
testTS = T(testIndices);

labels = LabC060314{1:100:end,4} == positiveClass; %binary labels of positive class
labels = labels(testIndices);
...
%%% Get Motif As Query %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%
[mp,mpi] = mpx_v3(pos, ceil(subLength/2), subLength);
...
for queryIndex = 1:size(queries,1)
    ...
    distanceProfile = MASS_V2(testTS,query);

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% Select Top K Indices %%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%
    K = 1000;
    bestIndices = allLowestDistanceIndices_V07(distanceProfile, subLength, ceil(subLength/2), K);
    ...
    ax2 = nexttile();
    plot(labels);
    titleFormat = sprintf("Labels, Class Positive = %s",positiveClass);
    title(titleFormat);
    ylim([-0.1, 1.1]);
