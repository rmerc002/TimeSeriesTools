...
%%%    Make the time series stationary
    ...
    numDataSetsPerClass = 1; %%%TODO: make this 5
    numDataSamples = 5;
    %%% temp for testing
    curatedDataPath = "C:\Users\Ryan\Dropbox\Education\UCR\Research\0_Projects\ActiveResearch\shapeletDiscovery\curatedData";
    datasets = {};
    datasets{length(datasets)+1} = {"D:\Datasets\UCRArchive_2018\UCRArchive_2018\GestureMidAirD1\GestureMidAirD1_TRAIN.tsv","GestureMidAirD1","train","segmented",false,0,1,2,-1};
    datasets{length(datasets)+1} = {"D:\Datasets\UCRArchive_2018\UCRArchive_2018\GestureMidAirD1\GestureMidAirD1_TEST.tsv","GestureMidAirD1","test","segmented",false,0,1,2,-1};
    ...
    for di = 1:length(datasets)
        ...
        %%% insert mean values for nan
        if sum(isnan(data),'all') > 0
           nanWarningFilePath = fullfile(proposedDirsLevel3(1), proposedDirsLevel3(2), proposedDirsLevel3(3), '_WARNING_NAN.txt');
           if ~isfile(nanWarningFilePath)
                fclose(fopen(nanWarningFilePath,'w'));
           end
           nanWarningFilePath = fullfile(proposedDirsLevel3(1), proposedDirsLevel3(2), proposedDirsLevel3(3), '_WARNING_NAN');
           if ~isfolder(nanWarningFilePath)
                mkdir(nanWarningFilePath);
           end
        end
        for dataIndex = 1:size(data,1)
            data(dataIndex,isnan(data(dataIndex,:))) = nanmean(data(dataIndex,:),2);
        end
        ...
        for classIndex = 1:1%numClasses %%%TODO: comment this back in
...
%                     part1 = part1 - part1(1) + positiveTS(end);
                    ...
                    signalPart = positiveSamples(i,:);
                    signalPart(isnan(signalPart)) = nanmean(signalPart);
                    signalPart = zscore(signalPart);
                    prevRandSTD = nanstd(positiveTS(end-signalLength+1:end));
%                     figure; plot(positiveTS(end-signalLength+1:signalLength));
                    ...
                    positiveTS = [positiveTS, signalPart ,randomPart];
