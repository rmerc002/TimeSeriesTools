function datasetCreation_V06(datasets, curatedDataPath)
%%% Purpose:
...
%tested above (quickly)
                ...
                pregeneratedBackgroundData = rand(1,ceil(fillerLength*numDataSamples*1.1));%extra just so I don't have to be precise in the calculation below
                ...
                fprintf("randomWalkLength: %d, preGenLength: %d\n",randOffset(1), length(pregeneratedBackgroundData));

                positiveTS = pregeneratedBackgroundData(1:randOffset(1));
                pregeneratedBackgroundData(1:randOffset(1)) = [];
                for i = 1:numDataSamples
                    ...
                    if i < numDataSamples
                        ...
                        fprintf("randomWalkLength: %d, preGenLength: %d\n",randomWalkLength, length(pregeneratedBackgroundData));
                        randomPart = pregeneratedBackgroundData(1:randomWalkLength);
                        pregeneratedBackgroundData(1:randomWalkLength) = [];
                ...
                if trainTest == "train"
                    negativeTS = rand(1,length(positiveTS));
                else
                   netativeTS = rand(1,signalLength);%just some dummy data, should not be used. 
                end
