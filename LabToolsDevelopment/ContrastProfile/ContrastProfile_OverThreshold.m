function [platos, platoIndices] = ContrastProfile_OverThreshold(positiveTS, negativeTS, m, threshold, forcePlot)
    %%% Input:
    ...
    %%%   threshold: Selects behaviors with contrast greater than threshold
    ...
    %%%   platoIndices: The starting index of each of the Platos
    ...
    platos = nan(1,m);
    platoIndices = nan(1,1);
    platoPrimaryContrast = nan(1,1);
    platoNaryContrast = nan(1,1);
%     CPs = nan(1,length(MP_AA));
%     primaryCP = nan(1,length(MP_AA));
    
    MP_AB_history = sqrt(2*m)*ones(length(MP_AA), 1);
    pastPlato = negativeTS;
    ki = 1;
    while true
        %%% Matrix profile AB-join between positiveTS and negativeTS
        [MP_AB, MP_AB_Indices] = mpxABBA(positiveTS, pastPlato, m);
        MP_AB = real(MP_AB);
        %%% Euclidean distance values above sqrt(2*m) are equivalent to
        %%%   anti-correlated values
        ...
        nanIndices = isnan(MP_AB);
        
        MP_AA(nanIndices) = nan;
        MP_AB_history = nanmin(MP_AB_history, MP_AB);
        %%% Contrast Profile
        CP = MP_AB_history - MP_AA;
...
%         platoPrimaryContrast(ki) = primaryCP(platoIndex); 
        %This may be helpful in identifying diminishing returns/redundant platos
%         platoNaryContrast(ki) = maxContrastValue; 
        platos(ki,:) = plato;
     
        %%% setup for next iteration
        exclusionLength = m;
        startIndex = max(1, platoIndex-exclusionLength);
        endIndex = min(length(positiveTS), ceil(platoIndex + m - 1 + exclusionLength));
        pastPlato = positiveTS(startIndex:endIndex);

        ki = ki + 1;
%         positiveTS(startIndex:endIndex) = nan;
        
%         fprintf("Found plato for K=%d at index %d with contrast value: %.2f\n", ki, platoIndex, maxContrastValue);
    end 
    ...
    if forcePlot == true
        ...
        hold off;
        formattedTitle = sprintf("Top-%d Platos", K);
        ...
        %%% Visualize contrast decay for increasing value of K
        %%% Usefullness of this plot is unclear
        figure;
        hold on;
        for xIndex = 1:K
           plot([xIndex, xIndex], [platoPrimaryContrast(xIndex),platoNaryContrast(xIndex)],'Color',[0.5,0.5,0.5]);
        end
        scatter(1:K, platoNaryContrast,'filled','MarkerFaceColor',colors(2,:));
        scatter(1:K, platoPrimaryContrast,'filled','MarkerFaceColor',colors(1,:));
        hold off;
        ylim([-0.1,1.1]);
        xlabel("K index");
        ylabel("Contrast");
        title("Assessing Contrast Decay");
    end
end
