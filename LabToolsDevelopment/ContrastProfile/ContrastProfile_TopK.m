function [platos,platoIndices, platoPrimaryContrast, platoNaryContrast] = ContrastProfile_TopK(positiveTS, negativeTS, m, K, forcePlot)
    %%% Input:
    ...
    %%%   K: Number of unique behaviors to search for. Minimally affects
    %%%     running time. Choosing large K requires user to weed out
    %%%     redundant behaviors.
    ...
    platos = nan(K,m);
    platoIndices = nan(K,1);
    platoPrimaryContrast = nan(K,1);
    platoNaryContrast = nan(K,1);
    CPs = nan(K,length(MP_AA));
