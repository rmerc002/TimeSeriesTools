function [platos, platoIndices, platoContrast, CP] = LeftContrastProfile(positiveTS, negativeTS, m, contrastThreshold, forcePlot)
    %%% Input:
...
%     [MP_AA, MP_AA_Indices] = mpxSelfJoin(positiveTS, ceil(m/2), m);
    [~, MP_AA, ~, MP_AA_Indices] = mpxLeftRight(positiveTS, ceil(m/2), m);
    MP_AA = real(MP_AA);
    ...
    padLength = length(positiveTS) - length(MP_AA);
    MP_AA = [MP_AA;NaN(padLength,1)];

    %%%Continue here 2021-12-13
    ...
    CP = zeros(1, numSubsequences);

    MP_AB = nan(length(positiveTS),1);
    tempMP_AB = mpxABBA(positiveTS, negativeTS, m);
    MP_AB(1:length(tempMP_AB)) = clipMatrixProfileAmplitude(tempMP_AB, m);

    CP = MP_AB - MP_AA;
    CP = normalizeContrastProfileAmplitude(CP, m);
    
    for timeIndex = m+1: numSubsequences - m %%% -m for context
        ...
        for platoIndex = 1:numPlatos
            ...
            if platoIndex < numPlatos || timeIndex > platoIndices(platoIndex) + m
        ...
        if ~isempty(platosWithContext) && timeIndex < platoIndices(end) + m && CPi > platoContrast(end)
