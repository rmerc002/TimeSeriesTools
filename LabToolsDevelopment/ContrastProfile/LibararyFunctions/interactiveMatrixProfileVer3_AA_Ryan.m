...
function [matrixProfiles, percentages] = ...
    interactiveMatrixProfileVer3(data, subLen, anytimeMode)
%% Contrast Profile AB-join Workaround%+Ryan+
rng(1);
proLen = length(data) - subLen + 1;%+Ryan+

timeSlice = 0.005;%+Ryan+%out of 100
numIterations = 0;%+Ryan+
matrixProfiles = (sqrt(4*subLen))*ones(1,proLen);%+Ryan+
percentages = [0];%+Ryan+
...
for i = 1:iterationEnd
    ...
    if numIterations / totalIterations >= length(percentages)*timeSlice%+Ryan+
        matrixProfiles(end+1,:) = matrixProfile(1:proLen);%+Ryan+
