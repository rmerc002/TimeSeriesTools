function [mp_a, mp_b, mpi_a, mpi_b, snapshots_mp_a, snapshots_mpi_a] = mpx_ABBA_v2_instrumented(a, b, w)
...
snapshotStepSize = 0.5;%Used by the anytime simulation
rng(1);
...
red_subcount_a = length(a) - w + 1;
red_subcount_b = length(b) - w + 1;

invn_a = NaN(red_subcount_a, 1);
invn_b = NaN(red_subcount_b, 1);
...
% Max compare count. Doesn't account for internal missing data
% sections since we have to traverse these anyway
cmpct = red_subcount_a * red_subcount_b;

numSnapshots = ceil(100/snapshotStepSize)+1;
snapshots_mp_a = {zeros(length(mp_a),1)};
snapshots_mpi_a = {zeros(length(mp_a),1)};
diagct = red_subcount_a + red_subcount_b;
ordering = randperm(diagct);
perc_compl = 0;
cmp_compl = 0;
...
for i = 1 : diagct
    d = ordering(i);
    if d <= red_subcount_a
        [mp_a, mp_b, mpi_a, mpi_b] = run_mpx(a, b, mu_a, mu_b, invn_a, invn_b, df, dg, dx, dy, mp_a, mp_b, mpi_a, mpi_b, w, d);
        cmps = min(red_subcount_a - d, red_subcount_b);
    else
        index = d - red_subcount_a;
        [mp_b, mp_a, mpi_b, mpi_a] = run_mpx(b, a, mu_b, mu_a, invn_b, invn_a, dx, dy, df, dg, mp_b, mp_a, mpi_b, mpi_a, w, index);
        cmps = min(red_subcount_b - index, red_subcount_a);
    end
    cmp_compl = cmp_compl + cmps;
    p = cmp_compl / cmpct * 100;
    if p >= (length(snapshots_mp_a))*snapshotStepSize
        snapshots_mp_a{end+1} = sqrt(2 * w * max(0, 1 - mp_a, 'includenan'));
        snapshots_mpi_a{end+1} = mpi_a;
        perc_compl = p;
        disp(p);
    end
end
...
function [mpa, mpb, mpia, mpib] = run_mpx(a, b, mua, mub, invna, invnb, df, dg, dx, dy, mpa, mpb, mpia, mpib, w, indices)
amx = length(a) - w + 1;
bmx = length(b) - w + 1;
