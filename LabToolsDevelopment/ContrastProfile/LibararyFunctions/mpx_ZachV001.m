function [matrixProfile, matrixProfileIdx, isvalidwindow, motifsIdx, discordsIdx] = mpx_ZachV001(timeseries, minlag, subseqlen, thresh, plot_output)
...
matrixProfile = zeros(subcount,1);
...
for diag = minlag + 1 : subcount
    ...
    for row = 1 : subcount - diag + 1
        ...
        if corr_ > thresh
            matrixProfile(row) = matrixProfile(row) + corr_;
            matrixProfileIdx(row) = matrixProfileIdx(row) + 1;
        end
        if corr_ > thresh
            matrixProfile(col) = matrixProfile(col) + corr_;
            matrixProfileIdx(col) = matrixProfileIdx(col) + 1;
        end
    end
end
