...
function [matrixProfiles, percentages] = ...
    interactiveMatrixProfileVer3_AB_Ryan(dataA, dataB, subLen, anytimeMode)
%% Contrast Profile AB-join Workaround %+Ryan+
rng(1);
if size(dataA,1) ~= 1
    dataA = dataA';
end
...
data = [dataA, NaN, dataB];
dataLenA = length(dataA);
dataLenB = length(dataB);
proLenA = dataLenA - subLen + 1;
proLenB = dataLenB - subLen + 1;

timeSlice = 0.005;%out of 100
numIterations = 0;
matrixProfiles = (sqrt(4*subLen))*ones(1,proLenA);%+Ryan+
percentages = [0];
...
firstUpdate = true;
if anytimeMode ==1 || anytimeMode ==2 || anytimeMode ==4
...
%     if idx <= dataLenA%+Ryan+
%         continue%+Ryan+
%     end%+Ryan+
%     numIterations = numIterations + 1;%+Ryan+
    
    if anytimeMode == 1 || anytimeMode ==4
        if idx > dataLenA%+Ryan+
            continue%+Ryan+
        end%+Ryan+
        numIterations = numIterations + 1;%+Ryan+
...
%         end%+Ryan+
        ...
        updatePos = (matrixProfile(pos1) > distProfile) & (pos2 > dataLenA)';
        profileIndex(pos1(updatePos)) = pos2(updatePos);
        matrixProfile(pos1(updatePos)) = distProfile(updatePos);
        
        updatePos = (matrixProfile(pos2) > distProfile) & (pos1 > dataLenA)';
    ...
    totalIterations = (length(idxOrder)-sum(isSkip));%+Ryan+
    if numIterations / totalIterations >= length(percentages)*timeSlice%+Ryan+
        matrixProfiles(end+1,:) = matrixProfile(1:proLenA);%+Ryan+
        percentages(end+1) = numIterations / totalIterations;%+Ryan+ upper bound on timeSlice is 100, like 100%
    end
...
matrixProfile(proLenA+1:end) = [];
matrixProfiles(end+1,:) = matrixProfile(1:proLenA);%+Ryan+
percentages(end+1) = 1;%+Ryan+ upper bound on timeSlice is 100, like 100%
