function [matrixProfile, matrixProfileIdx, isvalidwindow, snapshots_mp, snapshots_mpi] = mpx_v3_instrumented(timeseries, minlag, subseqlen, plot_output)
...
%
% Edited: 1/13/21: Dropped returns of motifs and discords. Added snapshots
% of partial execution for anytime examples.
%
...
% count total number of unique comparisons
% which is the sum of 1 to subcount - minlag
cmpct = (subcount - minlag) * (subcount - minlag + 1) / 2;
numSnapshots = ceil(100/snapshotStepSize)+1;
snapshots_mp = {zeros(length(matrixProfile),1)};
snapshots_mpi = {zeros(length(matrixProfile),1)};
ordering = randperm(subcount - minlag) + minlag;
perc_compl = 0;
cmp_compl = 0;
...
for d = 1 : length(ordering)
    ...
    cmp_compl = cmp_compl + subcount - diag + 1;
    p = cmp_compl / cmpct * 100;
    if p >= (length(snapshots_mp))*snapshotStepSize
        snapshots_mp{end+1} = sqrt(max(0, 2 * subseqlen * (1 - matrixProfile), 'includenan'));
        snapshots_mpi{end+1} = matrixProfileIdx;
        perc_compl = p;
        disp(p);
    end
end
