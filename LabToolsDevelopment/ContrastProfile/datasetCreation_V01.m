function datasetCreation_V01(datasets, curatedDataPath)
%%% temp for testing
...
datasets{length(datasets)+1} = {"D:\Datasets\UCRArchive_2018\UCRArchive_2018\Mallat\Mallat_TEST.tsv","Mallat","test","segmented",false,0,1,2,1025};
    ...
    for di = 1:length(datasets)
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%
        numDataSetsPerClass = 5; %%%TODO: make this 5
        numDataSamples = 5;
...
%tested above (quickly)
                ...
                positiveTS = zscore(getRandWalk(randOffset(1)));
...
%                     part1 = part1 - part1(1) + positiveTS(end);
                    ...
                    if i < numDataSamples
                        ...
                        randomPart = zscore(getRandWalk(fillerLength - currentRandomOffset + nextRandomOffset));
                        randomPart = randomPart - randomPart(1) + signalPart(end);
                    else
                        randomPart = zscore(getRandWalk(fillerLength - currentRandomOffset));
                        randomPart = randomPart - randomPart(1) + signalPart(end);
                    end
